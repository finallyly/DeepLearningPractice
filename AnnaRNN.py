...
def get_batches(arr,n_seqs, n_steps):
    #Get the number of characters per batch, and the number of batches we can make
    characters_per_batch = n_seqs*n_steps;
    n_batches =len(arr)/characters_per_batch;
    # Keep only enough characters to make full batches
    arr=arr[:n_batches*characters_per_batch];

    #Reshape into n_segs rows
...
def build_inputs(batch_size, num_steps):
    ...
    #Kepp probability placeholder for drop out layers
...
class CharRNN:
    def __init__(self,num_classes, batch_size=64,num_steps=50,
                       lstm_size=128, num_layers=2,learning_rate=0.01,
...
def pick_top_n(preds, vocab_size,top_n=5):
    ...
    p=p/np.sum(p);
    c=np.random.choice((vocab_size,1,p))[0];
    return c;

def sample(checkpoint, n_samples, lstm_size,vocab_size,prime="The "):
    samples = [c for c in prime];
    model = CharRNN(num_classes=(vocab),lstm_size=lstm_size,sampling=True);
