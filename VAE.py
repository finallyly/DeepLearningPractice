...
class VariantionalAutoencoder(object):
    ...
    def build(self):
        ...
        f3 = fc(f2,256,scope='enc_f3',activation_fn=tf.nn.elu);
        self.z_mu = fc(f3,self.n_z,scope="enc_f4_mu",activation_fn=None);
        self.z_log_sigma_sq=fc(f3,self.n_z,scope="enc_f4_sigma",activation_fn=None);
        ...
        latent_loss = -0.5*tf.reduce_sum(1+self.z_log_sigma_sq-tf.square(self.z_mu)-tf.exp(self.z_log_sigma_sq),1);
    ...
    def  run_single_step(self,x):
        _,loss,recon_loss, latent_loss = self.sess.run([self.train_op,self.total_loss,
                                                        self.recon_loss,self.latent_loss],feed_dict={self.x:x});
...
def main():
    ...
    # Train the model with 2d later space;
